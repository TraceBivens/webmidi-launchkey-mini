...
function () {
  ...
  ScopeRenderer.prototype.drawGridlines = function() {
    ...
    for (var i = 1; i < numVerticalLines; i++) {
      var x = i * verticalSpacing + this.gridOffsetH * verticalSpacing / 10;
      this.ctx.beginPath();
      this.ctx.moveTo(x, 0);
      this.ctx.lineTo(x, this.height);
      this.ctx.stroke();
      
      // Time labels (assuming 44.1kHz sample rate, 8192 samples)
      var timeMs = ((i + this.gridOffsetH / 10) * 8192 / 44100 * 1000 / numVerticalLines);
      this.ctx.fillText(timeMs.toFixed(1) + 'ms', x + 2, 12);
    }
    ...
    for (var i = 1; i < numHorizontalLines; i++) {
      var y = i * horizontalSpacing + this.gridOffsetV * horizontalSpacing / 8;
      this.ctx.beginPath();
      this.ctx.moveTo(0, y);
      ...
      var amplitude = (1 - (2 * (i + this.gridOffsetV / 8) / numHorizontalLines));
      this.ctx.fillText(amplitude.toFixed(1), 2, y - 2);
    }
    ...
    if (this.gridOffsetH !== 0 || this.gridOffsetV !== 0) {
      ...
      this.ctx.lineWidth = 2;
      var centerX = this.width / 2 + this.gridOffsetH * verticalSpacing / 10;
      var centerY = this.height / 2 + this.gridOffsetV * horizontalSpacing / 8;
  ...
  ScopeRenderer.prototype.moveGrid = function(deltaH, deltaV) {
    ...
    // Limit grid movement to reasonable bounds
    this.gridOffsetH = Math.max(-50, Math.min(50, this.gridOffsetH));
    this.gridOffsetV = Math.max(-32, Math.min(32, this.gridOffsetV));
  };
  ...
  ScopeRenderer.prototype.getGridPosition = function() {
    var timeMs = this.gridOffsetH * 8192 / 44100 * 1000 / 100; // Convert offset to time
    var amplitude = -this.gridOffsetV * 2 / 64; // Convert offset to amplitude
    return {
      timeMs: timeMs,
      amplitude: amplitude
    };
...
function () {
  ...
  ScopeSampler.prototype.triggerIndex = function (data) {
    var triggerLevel = 0.0; // 0V for float data (-1.0 to +1.0 range)
