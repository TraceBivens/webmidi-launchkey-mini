...
function () {
  ...
  ScopeRenderer.prototype.draw = function (sample) {
    ...
    for (var x = 0; sampleIdx < sample.data.length && x < this.width; sampleIdx++, x += xPxPerTimeSample) {
      // Apply amplitude multiplier and center around middle of canvas
      var normalizedSample = (sample.data[sampleIdx] - 128) * this.amplitudeMultiplier + 128;
      // Clamp to valid range
      normalizedSample = Math.max(0, Math.min(255, normalizedSample));
      // 0 => height, 128 => height/2, 255 => 0 - use minimal padding for maximum amplitude
      var magnitude = mapNumber(normalizedSample, 0, 255, this.height - 5, 5);
  ...
  ScopeRenderer.prototype.drawGridlines = function() {
    ...
    for (var i = 1; i < numVerticalLines; i++) {
      ...
      // Time labels (assuming 44.1kHz sample rate, 4096 samples)
      var timeMs = (i * 4096 / 44100 * 1000 / numVerticalLines).toFixed(1);
...
function () {
  function ScopeSampler(ac) {
    ...
    this.analyser = ac.createAnalyser(); // fftSize = 4096 => frequencyBinCount = 2048; 93ms of data per refresh at 44.1Khz (4096/44100)
