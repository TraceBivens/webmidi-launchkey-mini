...
function () {
  ...
  ScopeRenderer.prototype.draw = function (sample) {
    ...
    // Calculate scaling for time and voltage
    var samplesPerDiv = this.secDiv * 44100; // 44.1kHz sample rate
    var xPxPerSample = (this.width / this.gridDivisionsX) / samplesPerDiv;
    for (var x = 0, i = sampleIdx; i < sample.data.length && x < this.width; i++, x += xPxPerSample) {
      // Map 0-255 to voltage range, center at midline
      var v = (sample.data[i] - 128) / 128 * (this.gridDivisionsY / 2) * this.voltDiv;
      var magnitude = this.height / 2 - v * (this.height / this.gridDivisionsY) / this.voltDiv;
