...
function () {
  ...
  ScopeRenderer.prototype.drawGridlines = function() {
    ...
    for (var i = 1; i < numVerticalLines; i++) {
      ...
      // Time labels only on major gridlines
      if (i % 5 === 0) {
        this.ctx.fillStyle = '#999';
        var timeMs = ((i + this.gridOffsetH / 20) * 8192 / 44100 * 1000 / numVerticalLines);
    ...
    for (var i = 1; i < numHorizontalLines; i++) {
      ...
      // Amplitude labels only on major gridlines
      if (i % 4 === 0) {
        this.ctx.fillStyle = '#999';
        var amplitude = (1 - (2 * (i + this.gridOffsetV / 16) / numHorizontalLines));
  ...
  ScopeRenderer.prototype.getGridPosition = function() {
    var timeMs = this.gridOffsetH * 8192 / 44100 * 1000 / 200; // Convert offset to time with higher resolution
    var amplitude = -this.gridOffsetV * 2 / 128; // Convert offset to amplitude with higher resolution
    return {
      timeMs: timeMs,
      amplitude: amplitude
    };
