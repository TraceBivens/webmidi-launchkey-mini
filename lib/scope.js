...
function () {
  ...
  ScopeRenderer.prototype.displayFrequencyInfo = function(sample) {
    ...
    var crossings = 0;
    for (var i = 1; i < sample.data.length; i++) {
      if ((sample.data[i] >= 128 && sample.data[i-1] < 128) || 
          (sample.data[i] < 128 && sample.data[i-1] >= 128)) {
        crossings++;
      }
    }
    var estimatedFreq = (crossings / 2) * 44100 / sample.data.length;
    
    // Apply smoothing to frequency display
    if (!this.smoothedFreq) {
      this.smoothedFreq = estimatedFreq;
    } else {
      // Exponential moving average with alpha = 0.1 for stability
      this.smoothedFreq = 0.1 * estimatedFreq + 0.9 * this.smoothedFreq;
    }
    
    this.ctx.fillStyle = '#fff';
    this.ctx.font = '14px monospace';
    this.ctx.fillText('~' + this.smoothedFreq.toFixed(0) + ' Hz', this.width - 80, 20);
  };
